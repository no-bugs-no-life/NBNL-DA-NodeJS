...
module.exports = {
    getHomeData: async function (req, res, next) {
        try {
            ...
            let trendingApps = await appModel.paginate({ isDeleted: false, status: "published" }, { limit: 5, populate: 'categoryId' });
            let bestSellingGames = await productModel.paginate({ isDeleted: false }, { limit: 6 });
            let collections = await categoryModel.paginate({ isDeleted: false, parentId: null }, { limit: 2 });
            ...
            let productivityApps = await appModel.paginate({ isDeleted: false, status: "published" }, { sort: { createdAt: -1 }, limit: 3 });
