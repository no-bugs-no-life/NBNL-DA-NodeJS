...
function TopBar({ count, onAdd }: { count: number; onAdd: () => void }) {
  return (
    <div className="flex items-center justify-between mb-8">
      ...
      <button
        onClick={onAdd}
        className="flex items-center gap-2 bg-blue-100 hover:bg-blue-200 text-blue-800 px-5 py-2.5 rounded-xl text-sm font-semibold shadow-sm"
      >
    ...
    <>
      ...
      {s.showCreate && (
        <CategoryModal
          ...
          onSubmit={(name: string, iconUrl?: string) =>
            s.mCreate.mutate({ name, iconUrl })
          }
      ...
      {s.editTarget && (
        <CategoryModal
          title="Sửa"
          initialName={s.editTarget.name}
          initialIconUrl={s.editTarget.iconUrl}
          onClose={() => s.setEditTarget(null)}
          onSubmit={(n: string, iu?: string) =>
            s.mUpdate.mutate({ id: s.editTarget!._id, name: n, iconUrl: iu })
          }
