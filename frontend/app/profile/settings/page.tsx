...
function SettingsHeader() {
  return (
    <div className="mb-6">
      <h1 className="text-3xl font-black text-slate-800 tracking-tight">Cài đặt Tài khoản</h1>
      <p className="text-slate-500 mt-2 font-medium">Cập nhật thông tin cá nhân cơ bản và hình dáng hiển thị.</p>
    </div>
  );
}
...
export default function SettingsPage() {
  ...
  if (!user) return <div className="p-8 text-center text-slate-500 font-semibold animate-pulse">Đang tải cấu hình...</div>;
  ...
  return (
    <div className="max-w-3xl pt-2 pb-12 w-full animate-in fade-in duration-300">
      ...
      <div className="space-y-6">
        <ImageUploadForm user={user} onSave={(v: any) => updateMutation.mutate(v)} loading={updateMutation.isPending} />
        <BasicInfoForm user={user} onSave={(v: any) => updateMutation.mutate(v)} loading={updateMutation.isPending} />
        <SocialLinksForm user={user} onSave={(v: any) => updateMutation.mutate(v)} loading={updateMutation.isPending} />
      </div>
    </div>
  );
}
