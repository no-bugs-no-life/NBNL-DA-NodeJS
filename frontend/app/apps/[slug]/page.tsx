...
export default function AppDetailsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await api.get(`/api/v1/apps/slug/${slug}`);
  ...
  if (!data)
    return (
      <div className="h-screen flex items-center justify-center p-8 flex-col gap-4">
        <h1 className="text-3xl font-bold">App not found</h1>
        <p>
          The app you are looking for might have been removed or is temporarily
          unavailable.
        </p>
      </div>
    );
