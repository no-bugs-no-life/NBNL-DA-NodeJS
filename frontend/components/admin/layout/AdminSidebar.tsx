...
function SidebarItem({ menu, isActive, onClick }: { menu: { name: string; path: string; icon: React.ElementType }, isActive: boolean, onClick?: () => void }) {
    const Icon = menu.icon;
    return (
        <Link
            href={menu.path}
            onClick={onClick}
            className={`flex items-center gap-3 px-4 py-3 rounded-xl font-semibold mb-1 transition-all ${isActive
...
export function AdminSidebar({ isOpen, setIsOpen }: { isOpen?: boolean; setIsOpen?: (val: boolean) => void }) {
    ...
    return (
        <aside className={`w-64 border-r border-slate-200 bg-white h-screen shrink-0 overflow-y-auto p-4 flex-col fixed md:sticky top-0 z-50 shadow-[4px_0_24px_-12px_rgba(0,0,0,0.05)] scrollbar-hide transition-transform md:translate-x-0 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
