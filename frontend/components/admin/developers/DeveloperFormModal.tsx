...
export function DeveloperFormModal({ developer, onClose, onSubmit, loading }: Props) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white rounded-2xl w-full max-w-lg shadow-xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
                <div className="p-6 border-b border-slate-100 flex items-center justify-between">
                    <div>
                        <h2 className="text-xl font-bold text-slate-800">Chỉnh sửa Developer</h2>
                        <p className="text-xs text-slate-500 mt-1">Gắn với Account: {developer.userId?.email}</p>
                    </div>
                ...
                <div className="p-6 overflow-y-auto flex-1">
                    <form id="developer-form" onSubmit={handleSubmit} className="space-y-4">
                        <div>
                            <label className="block text-sm font-semibold text-slate-700 mb-1.5">Tên Developer</label>
                            <input
                                required
                                type="text"
                                value={formData.name}
                        ...
                        <div>
                            <label className="block text-sm font-semibold text-slate-700 mb-1.5">Tiểu sử (Bio)</label>
                            <textarea
                                rows={4}
                                value={formData.bio}
