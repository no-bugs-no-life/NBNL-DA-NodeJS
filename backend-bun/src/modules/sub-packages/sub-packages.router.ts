...
// Public Routes
subPackagesRouter.get("/", validateQuery(SubPackageQuerySchema), (c) => controller.list(c));
subPackagesRouter.get("/global", (c) => controller.getGlobal(c));
subPackagesRouter.get("/:id", validateParams(SubPackageParamsSchema), (c) => controller.getById(c));
subPackagesRouter.get("/app/:appId", (c) => controller.getByApp(c));

// Admin Routes
subPackagesRouter.post("/", requireAdmin, validateBody(CreateSubPackageSchema), (c) => controller.create(c));
...
subPackagesRouter.delete("/:id", requireAdmin, validateParams(SubPackageParamsSchema), (c) => controller.delete(c));
subPackagesRouter.patch("/:id/toggle", requireAdmin, validateParams(SubPackageParamsSchema), (c) =>
	controller.toggleActive(c),
);