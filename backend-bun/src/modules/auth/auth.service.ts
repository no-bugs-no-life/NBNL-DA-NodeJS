...
export class AuthService {
    ...
    async generateTokens(user: UserPayload): Promise<{ accessToken: string; refreshToken: string }> {
        const accessPayload = {
            ...user,
    ...
    async verifyRefreshToken(token: string): Promise<{ id: string; exp: number }> {
        try {
